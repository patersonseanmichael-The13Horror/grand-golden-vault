...
export function initSlots() {
  ...
  // Create a Canvas Texture for reels
  function createReelTexture(symbols, rows=6){
    const canvas = document.createElement('canvas');
    canvas.width = 128; canvas.height = 128*rows;
  ...
  // Reel spinning with texture offset
  function spinReelTexture(reelObj,spins=10,duration=2){
    const {texture} = reelObj;
    gsap.to(texture.offset,{y:spins,duration:duration,ease:"power4.inOut",onComplete:()=>{
